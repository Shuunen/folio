    ...
    <section id={slugify($t(messages.navigation.work))}>
      ...
      <div className="container mx-auto flex flex-col gap-14">
        ...
        <div className="mx-auto grid w-2/3 gap-14 md:mx-0 md:w-auto md:grid-cols-3">
          <ProjectCard color={tw('border-collectif-energie')} image={Collectif.src} subtitle={$t(collectifEnergie.subtitle)} title={$t(collectifEnergie.title)} />
          <ProjectCard color={tw('border-we-save')} image={WeSave.src} subtitle={$t(weSave.subtitle)} title={$t(weSave.title)} />
          <ProjectCard color={tw('border-lyf')} image={Lyf.src} subtitle={$t(lyf.subtitle)} title={$t(lyf.title)} />
          <ProjectCard color={tw('border-auchan')} image={Auchan.src} subtitle={$t(auchan.subtitle)} title={$t(auchan.title)} />
          <ProjectCard color={tw('border-enedis')} image={Enedis.src} subtitle={$t(enedis.subtitle)} title={$t(enedis.title)} />
          <ProjectCard color={tw('border-ouest-france')} image={OuestFrance.src} subtitle={$t(ouestFrance.subtitle)} title={$t(ouestFrance.title)} />
        </div>
      </div>
    </section>
  )
