 ...
 * @returns JSX.Element
 */
export function ProjectCardImage({ color, image, title }: Readonly<{ color: string; image: string; title: string }>) {
  return (
    <Tilt className="relative w-full max-w-[390px]" tiltMaxAngleX={5} tiltMaxAngleY={5}>
      ...
      <Image alt={title} class="pointer-events-none relative z-10" height="300" src={image} width="300" />
    </Tilt>
  )
}
